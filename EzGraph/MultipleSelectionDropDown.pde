...
class MultipleMainTab extends Obj{
	...
	void update(){
		...
		if(hovered && insideSelection(mouseX, mouseY) && !menu2.mainTabHovered()){
			int i;
			//do stuff if needed
		}
		else{
	...
	void addSubTab(String name, int index){
		subTabs.add(new MultipleSubTab(new PVector(seekPos.x, seekPos.y), seekCol, hoverCol, seekSize * 2/3, animSpd, name, index, seekW, seekH * 2/3));
	}
